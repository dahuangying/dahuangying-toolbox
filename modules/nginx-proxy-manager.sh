#!/bin/bash
# 大黄鹰-Nginx Proxy Manager 管理脚本 (兼容Docker/非Docker版)
# 版本: v2.1
# 功能: 安装/更新/卸载 (原生和Docker两种模式)
...
remove_firewall_rules() {
    ...
    echo -e "${GREEN}正在清理防火墙规则...${NC}"
    ufw status | grep -q "$port" && ufw delete allow "$port"
    ufw reload >/dev/null 2>&1
}

check_nginx_installed() {
    [ -d "$NPM_DIR" ] && return 0 || return 1
}
...
show_menu() {
    clear
    echo -e "${GREEN}=== Nginx Proxy Manager 管理脚本 ==="
    echo -e "当前模式: ${RED}$([ -f "$NPM_DIR/docker-compose.yml" ] && echo "Docker版" || echo "原生版")${NC}"
    echo "1. 安装原生版"
    echo "2. 更新原生版"
    echo "3. 卸载原生版"
    echo "4. 安装Docker版"
